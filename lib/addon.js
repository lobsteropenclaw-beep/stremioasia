...
const manifest = {
  id: 'community.stremio.asian-discovery-v3',
  version: '3.0.0',
  name: 'Asian Discovery ULTIMATE',
  description: 'Premium Korean, Chinese, and Japanese catalogs. Fully compatible with stream scrapers.',
...
builder.defineMetaHandler(async ({ type, id }) => {
  ...
  if (item) {
    ...
    const externalId = item.external_ids?.imdb_id;
    
    const meta = {
      id: externalId || id, // If IMDb ID exists, use it!
      type: type,
      name: item.title || item.name,
      poster: item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : null,
      background: item.backdrop_path ? `https://image.tmdb.org/t/p/original${item.backdrop_path}` : null,
