...
const manifest = {
  id: 'community.stremio.asian-discovery-v8',
  version: '8.0.0',
  name: 'Asian Discovery ULTIMATE + TRENDING',
  description: 'Premium catalogs with Trending lists for both Movies and Dramas. Fully optimized for Real-Debrid.',
...
function mapToMeta(item, type) {
  return {
    id: `tmdb:${item.id}`,
    name: item.title || item.name, // Keep Chinese title for catalog browsing
    type: type,
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
  ...
  if (id === 'trending_movie' || id === 'trending_drama') {
...
builder.defineMetaHandler(async ({ type, id }) => {
  ...
  // Fetch details in English specifically for the meta name to help scrapers
  ...
  if (item) {
    ...
    const meta = {
      id: imdbId || id,
      type: type,
      // CRITICAL: We use the English title as the main Name because scrapers (Torrentio) 
      // rely on this for searching if the IMDb ID cache is cold.
      name: englishItem?.title || englishItem?.name || item.title || item.name,
      poster: item.poster_path ? `https://image.tmdb.org/t/p/w500${item.poster_path}` : null,
      background: item.backdrop_path ? `https://image.tmdb.org/t/p/original${item.backdrop_path}` : null,
